...
5. Return ONLY raw JSON. No markdown formatting.`;
  ...
  try {
    ...
    // Fix: Remove markdown code fences if present to prevent JSON.parse failure
    const cleanText = text.replace(/```json\n?|\n?```/g, '').trim();
    
    if (!cleanText) throw new Error("No config generated");
    
    return JSON.parse(cleanText) as AdminConfig;
  } catch (error) {
...
export const generateAIResponse = async (
  ...
  if (mode === 'creative' && images.length === 0) {
    try {
      ...
      // Enable googleSearch tool for grounding as requested
      const response = await ai.models.generateContent({
