...
import { Send, Image as ImageIcon, X, Loader2, Bot, User as UserIcon, Globe, Sparkles, Brain, Zap, Download } from 'lucide-react';
               ...
               <div className={`max-w-[85%] md:max-w-[75%] rounded-2xl p-4 shadow-md ${
                 ...
                 {msg.generatedImage && (
                   <div className="mt-3 relative group inline-block">
                     ...
                     <button 
                       onClick={() => downloadImage(msg.generatedImage!, `hk-ai-gen-${msg.id}.png`)}
                       className="absolute top-2 right-2 p-2 bg-slate-900/70 hover:bg-slate-900 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all backdrop-blur-sm border border-white/20 transform scale-90 group-hover:scale-100 shadow-xl"
